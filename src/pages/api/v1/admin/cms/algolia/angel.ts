    ...
    switch (req.method) {
      case 'POST': {
        if (!req.body || Object.keys(req.body).length === 0) {
          return res.status(400).json({ message: 'Angel data is required.' });
        }
        ...
        res.status(200).json({
          message: 'Angel synced with Algolia successfully',
          data: record,
        });
        break;
      }
      case 'DELETE': {
        ...
        res.status(200).json({
          message: 'Angel deleted from Algolia successfully',
        });
        break;
      }
      default:
